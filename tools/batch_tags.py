...
レジュームできない。そこで shard の有無に関係なく全未分類問題から切り、済み判定は
shard ファイルの存在だけで行う。

**この境界は classify.py の出力に依存する。** 静的解析ルールを足すと未分類集合が変わり、
バッチの中身も変わる。放置すると同じ問題が別々の shard に二重に入ったり、どの shard にも
入らず消えたりする。これを人間の注意力に任せず、未分類 ID 一覧の指紋を `_manifest.json` に
置いて毎回照合する（ズレたら止まる）。
...
class Coverage:
    ...
    missing: set[str] = frozenset()  # shard に現れなかった問題（truncate や境界ズレで起きる）
    unexpected: set[str] = frozenset()  # このバッチに無いのに shard に現れた問題
