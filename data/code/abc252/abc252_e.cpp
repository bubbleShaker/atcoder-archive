...
int main(){
  ...
  vector<pair<long long,int>> cur(n+1,{INF,0});//暫定距離を記録
  vector<bool> confirmed(n+1);//最短距離が確定した頂点
  auto dijkstra=[&](){
    priority_queue<tuple<long long,int,int>,vector<tuple<long long,int,int>>,greater<tuple<long long,int,int>>> hque;
    hque.push({0,1,1});
    cur[1]={0,1};
    while(!hque.empty()){
      auto [_,u,t]=hque.top();
  ...
  for(int i=2;i<=n;i++){
    ans.push_back(mp[{i,cur[i].second}]);
  }
  for(auto &&num:ans){
    cout<<num<<' ';
  }
