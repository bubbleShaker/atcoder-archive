...
namespace AtCoderBeginnerContest162.Questions
{
    public class QuestionA : AtCoderQuestionBase
    {
        ...
        {
            var (n, q) = TextReaderExtensions.ReadValue<int, int>(inputStream);
            var dsu = new AtCoder.Dsu(n);
            for(int i = 0; i < q; i++)
            {
                var (t, u, v) = TextReaderExtensions.ReadValue<int, int, int>(inputStream);
                if (t == 0)
                {
                    dsu.Merge(u, v);
                    continue;
                }
                yield return dsu.Same(u, v) switch
                {
                    true => 1,
                    false => 0
                };
