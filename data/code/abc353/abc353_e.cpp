...
struct Trie {
    ...
    int64_t dfs_common_sum(int node_id) {
        //cerr << "Node ID: " << node_id << ", Character: " << nodes[node_id].c << ", Common: " << nodes[node_id].common << endl;
        int64_t n=nodes[node_id].common;
        int64_t sum = (n-1)*n/2;
        if(node_id==0){
            sum=0;
        }
        for (int i = 0; i < char_size; ++i) {
            if (nodes[node_id].next[i] != -1) {
                sum += dfs_common_sum(nodes[node_id].next[i]);
            }
        }
