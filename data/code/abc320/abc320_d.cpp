...
int main(){
    ...
    vector<lP> tyoten(n);
    rep(i,n)tyoten[i]=lP(-INFL,-INFL);
    tyoten[0]=lP(0,0);
    
    auto dfs=[&](auto dfs,Int from)->void{
        visited[from]=true;
        
        for(auto [to,dx,dy]:g[from]){
            if(visited[to])continue;
            tyoten[to].first=tyoten[from].first+dx;
            tyoten[to].second=tyoten[from].second+dy;
            dfs(dfs,to);
        }
    };
    ...
    rep(i,n){
        if(visited[i]){
            cout<<tyoten[i].first<<" "<<tyoten[i].second<<endl;
        }else{
