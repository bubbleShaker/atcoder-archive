...
struct Clock {
    ...
    string to_str(){
        ...
        s_s = to_string(second);
        
        if(h_s.length() == 1) h_s = "0" + h_s;
    ...
    void shift(int diff_second){
        ...
        h_d = abs(diff_second) / (60*60);
        m_d = (abs(diff_second) / 60) % 60;
        s_d = abs(diff_second) % 60;
        
        if(diff_second >= 0){
            if((second + s_d) >= 60){
                m_d++;
            }
            second = (second + s_d) % 60;
            if((minute + m_d) >= 60){
                h_d++;
            }
            minute = (minute + m_d) % 60;
            hour = (hour + h_d) % 24;
        }else{
            if((second - s_d) >= 0) second = second - s_d;
            else {
                second = second - s_d + 60;
                m_d++;
            }
            
            if((minute - m_d) >= 0) minute = minute - m_d;
            else {
                minute = minute - m_d + 60;
                h_d++;
            }
                
            if((hour - h_d) >= 0) hour = hour - h_d;
            else hour = hour - h_d + 24;
